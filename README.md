A package to bridge actions between ROS1 and ROS2. 

Please clone `jazzy-devel` branch in case you are working with ROS Jazzy.

**NOTE:**   
- Currently supports forwarding goals from ROS1 (one) action client to ROS2 (jazzy) action server  
- As an example, implemented interfaces for the action bridge for AudioAction

**Prerequisites:**  

(*rosdep does not work properly with mixed ROS1 and ROS2 dependencies*)

```
sudo apt install ros-one-actionlib ros-one-actionlib-tutorials ros-one-control-msgs ros-one-roscpp 
ros-jazzy-control-msgs ros-jazzy-example-interfaces ros-jazzy-rclcpp ros-jazzy-rclcpp-action
```

**How to build:**  
  
Clone the repository in the `src` folder of your ROS2 workspace.
```
git clone git@github.com:ipa-hsd/action_bridge.git
```

Since `action_bridge` package depends on both ROS1 and ROS2, source both workspaces.
```
source /opt/ros/one/local_setup.bash
source /opt/ros/jazzy/local_setup.bash
colcon build --symlink-install --packages-select action_bridge
```
Now you are ready to run the `action_bridge`!  
Source this workspace to use the executeables built in the previous step. 
```
source <path-to-workspace>/install/local_setup.bash
```
2 example executables are available: 
- `action_bridge_fibonacci_node` and 
- `action_bridge_ue_audio_node`  
You can start one of these nodes in the following manner:
```
ros2 run action_bridge action_bridge_fibonacci_node
```
OR
```
ros2 run action_bridge action_bridge_ue_audio_node
```









